---
layout: post
title:  "JSON Web Tokens in django application - part IV"
date:   2017-08-20
author:     "Kyle Ding"
...
In this view, I simply add few additional lines for creating JWT.  First I created payload by adding `user` to JWT creation process, then I created the token from payload by calling `jwt_encode_handler`. At the end instead of returning user data, I return `confirmation_url` for the end user to enter and activate the account. By default django make every user active so I have to write my own `create` method for `UserSerializer`:
...
That's all for today! Feel free to comment and check repo for this blog post under this [link](https://github.com/kylemyding/django-jwt).